...
@model(
    ...
    kind=dict(name=ModelKindName.INCREMENTAL_BY_TIME_RANGE, time_column="ts"),#, batch_size=24, batch_concurrency=1),
    # interval_unit="hour",
    cron="@daily",
...
def execute(
    ...
    **kwargs: Any,
    ...
    df1 = pl.from_pandas(context.fetchdf(f"SELECT * FROM {mean_cust_ret} {WHERE_CLAUSE}"))
    df2 = pl.from_pandas(context.fetchdf(f"SELECT * FROM {cust_cost_spread} {WHERE_CLAUSE}"))
